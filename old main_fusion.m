fusion_type = ["simple_average", "Weighted_average_SWICW", "max_POB_per_sample"];%, "Weighted_averageSTD_POB_per_sample",...
...
for count_w_set = 1:length(all_Ws_set)
    ...
    if ~exist('W', 'var')
        ...
        for K = 1 : W_num
            ...
            load(strcat(strrep(Ws_for_fusion.Ws(K).W_folder,'E:\Arshad\payan','C:'),...
                '\', Ws_for_fusion.Ws(K).name));
            W{K} = W_graph.W;
    ...
    for f_counter = 1:length(fusion_type)
        ...
        %         W_name = '';% aval error hesab shavad to dar name w bashad!
        % propgation NEW Fusion W and calculate error
        ...
        result_W_fusion(f_counter) = W_test_error_only(W_graph_temp, labled_num, splits_num, fusion_type(f_counter), W_folder);
        % for w name need: lgc_errors_mean_L, grf_errors_mean_L
        eval(strcat('lgc_errors_mean_L= result_W_fusion(', num2str(f_counter),').lgc_L',num2str(labled_num(Def_L)),'_mean_errors;'));
        eval(strcat('grf_errors_mean_L= result_W_fusion(', num2str(f_counter),').grf_L',num2str(labled_num(Def_L)),'_mean_errors;'));
        W_name = strcat('W_Fusion_', strrep(fusion_type(f_counter), 'metric_fusion_','') ,'_',num2str(W_num), '_', dataset_name, '_',...
            'LGC_L', num2str(labled_num(Def_L)), '_', num2str(lgc_errors_mean_L),...
            '_GRF_L', num2str(labled_num(Def_L)), '_', num2str(grf_errors_mean_L));
...
%     Ws_for_fusion
    match = {};
    expression = ["Warning", "lgc_L\d+_errors", "grf_L\d+_errors", "POB_lgc", "W_folder"];
