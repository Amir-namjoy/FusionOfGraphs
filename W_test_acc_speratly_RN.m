function test_results=W_test_acc_speratly_RN(W_graph, labled_num, splits_num,...
    W_name, W_folder)
...
for count_labled=1:length(labled_num)
    for count_splits=1:splits_num
        ...
        try
            ...
            lgc_Accuracy = round((1-lgc_error)*100, 2);
            time_lgc = toc;
        ...
        try
            ...
            time_grf = toc;
            grf_Accuracy = round((1-grf_error)*100, 2);
            [warnMsg, warnId] = lastwarn;
            if ~isempty(warnId)
        ...
        eval(strcat('test_results.lgc_L',c_l,...
            '_Accuracy_split', c_s, '=',num2str(lgc_Accuracy),';'));
        
        eval(strcat('test_results.grf_L', c_l,...
            '_Accuracy_split', c_s, '=',num2str(grf_Accuracy),';'));
        
        %% eigenvalue & eigenvector
        %this function creat a matrix W of labaleds only
%         [W_labeled] = sparse(labeled_W_by_ind(W_graph.W, labeled_ind));
%         [W_WOCW_l_rem]=zero_WOCW_labeled_W_by_ind(W_graph.W, labeled_ind, W_graph.labels);
        [W_labeled] = labeled_W_by_ind(W_graph.W, labeled_ind);
        labels_labeled = W_graph.labels(labeled_ind);
        [W_labeled_NR, W_labeled_Noise] = Within_out_class_weigths(W_labeled, labels_labeled);
        %             labels_labeled
...
%         std_Noise_L = std(W_labeled_Noise(:));
        ...
        fprintf('=======> ');
        fprintf('lgc Accuracy = %f    ', lgc_Accuracy);
        fprintf('grf Accuracy = %f       ', grf_Accuracy);
